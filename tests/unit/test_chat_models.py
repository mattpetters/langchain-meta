...
def mock_llama_client_fixture():
    ...
    def create_mock_response_object():
        ...
        mock_completion_msg.content = {"type": "text", "text": "Mocked test response"} # Content can be str or MessageTextContentItem
        ...
        mock_tool_call.to_dict = MagicMock(return_value={
            ...
            "function": {"name": mock_tool_call_func.name, "arguments": mock_tool_call_func.arguments}
        })
        
        mock_completion_msg.tool_calls = [mock_tool_call]
        mock_completion_msg.stop_reason = "stop"
        mock_completion_msg.role = "assistant"
        mock_completion_msg.to_dict = MagicMock(return_value={
            "role": "assistant", "content": mock_completion_msg.content,
        ...
        # Mock metrics
        mock_metric_prompt = MagicMock(); mock_metric_prompt.metric="num_prompt_tokens"; mock_metric_prompt.value=20
        mock_metric_completion = MagicMock(); mock_metric_completion.metric="num_completion_tokens"; mock_metric_completion.value=40
        mock_metric_total = MagicMock(); mock_metric_total.metric="num_total_tokens"; mock_metric_total.value=60
        ...
        mock_response_obj.metrics = [mock_metric_prompt, mock_metric_completion, mock_metric_total]
        mock_response_obj.x_request_id = "mock_req_id_abc"
        mock_response_obj.to_dict = MagicMock(return_value={
    ...
    with patch("llama_api_client.LlamaAPIClient") as mock_sync_client_constructor, \
        ...
        sync_create_method_mock = MagicMock(name="sync_create_method_mock", return_value=create_mock_response_object())
        ...
        def sync_create_side_effect(*args, **kwargs):
            if kwargs.get("stream"):
                ...
                mock_chunk1 = MagicMock(spec=CreateChatCompletionResponse) # Assuming stream yields these types
                mock_chunk1.completion_message = mock_chunk1_completion_msg
                mock_chunk1.usage = None 
                mock_chunk1.x_request_id = "stream_req_id_1"
                mock_chunk1.to_dict = MagicMock(return_value={
                    "completion_message": {"content": "Streamed part 1", "tool_calls": None, "stop_reason": None},
                ...
                mock_stream_tool_func.arguments = "{\"param\":\"value\"}" # Corrected JSON string
                ...
                mock_stream_tool_call.to_dict = MagicMock(return_value={
                    "id": "stream_tool_id_1", "function": {"name": "stream_tool", "arguments": "{\"param\":\"value\"}"} # Corrected JSON string
                })
                ...
                mock_chunk2.usage = None # Usage might appear at the end or not at all in chunks
                mock_chunk2.x_request_id = "stream_req_id_2"
                mock_chunk2.to_dict = MagicMock(return_value={
                    "completion_message": {"content": " part 2", "tool_calls": [mock_stream_tool_call.to_dict()], "stop_reason": "tool_calls"},
                ...
                mock_final_chunk_completion_msg.content = None # Often empty in final chunk
                ...
                mock_final_usage.to_dict = MagicMock(return_value=mock_final_usage.__dict__)
                mock_final_chunk.usage = mock_final_usage 
                mock_final_chunk.x_request_id = "stream_req_id_final"
                mock_final_chunk.to_dict = MagicMock(return_value={
                    "completion_message": {"content": None, "tool_calls": None, "stop_reason": "stop"},
        ...
        sync_create_method_mock.side_effect = sync_create_side_effect # Apply the side effect
        ...
        async_create_method_mock = MagicMock(name="async_create_method_mock", side_effect=async_mock_create_side_effect)
        async_completions_attr.create = async_create_method_mock # Assign the async mock
...
def test_chat_meta_llama_initialization(configured_mock_llama_clients: MockClients): # Updated fixture name
    ...
    llm_with_sync_client = ChatMetaLlama(client=configured_mock_llama_clients.sync, api_key="test-key-ignored")
    ...
    llm_with_async_client = ChatMetaLlama(async_client=configured_mock_llama_clients.async_client, api_key="test-key-ignored-too")
    assert llm_with_async_client._async_client is configured_mock_llama_clients.async_client
    ...
    assert llm_with_both_clients._async_client is configured_mock_llama_clients.async_client
    # Here, _ensure_client_initialized should do nothing as both are provided.


@pytest.mark.asyncio
async def test_agenerate_method(configured_mock_llama_clients: MockClients): # Updated fixture name
    ...
    assert gen.generation_info["finish_reason"] == "stop" # from completion_message.stop_reason
    assert gen.generation_info["x_request_id"] == "mock_req_id_abc" # from response.x_request_id
    assert "usage_metadata" in gen.generation_info # From parsed metrics
    assert "response_metadata" in gen.generation_info # Should contain to_dict() of CreateChatCompletionResponse
    ...
    call_args = configured_mock_llama_clients.async_client.chat.completions.create.call_args
...
def test_tool_choice_parameter_filtering(configured_mock_llama_clients: MockClients):
    ...
    assert "max_completion_tokens" in call_args.kwargs # Check for the Llama API parameter name
    assert call_args.kwargs["max_completion_tokens"] == 100 # Check for the Llama API parameter name
    assert call_args.kwargs['model'] == "TestModel"
    assert call_args.kwargs['messages'] == [{'role': 'user', 'content': 'Hello'}]

@pytest.mark.asyncio
async def test_async_tool_choice_parameter_filtering(configured_mock_llama_clients: MockClients):
    ...
    await llm._agenerate(messages, tool_choice="required", temperature=0.5, max_tokens=200)
    ...
    assert "max_completion_tokens" in call_args.kwargs # Check for the Llama API parameter name
    assert call_args.kwargs["max_completion_tokens"] == 200 # Check for the Llama API parameter name
    assert call_args.kwargs['model'] == "TestModel"
    assert call_args.kwargs['messages'] == [{'role': 'user', 'content': 'Hello'}]

@pytest.mark.asyncio
async def test_bind_with_tool_choice_async(configured_mock_llama_clients: MockClients): # Updated fixture name
    ...
    assert bound_llm.kwargs["tools"] == [SimpleToolSchema], "Incorrect tools in bound_llm.kwargs"
    assert "tool_choice" in bound_llm.kwargs, "tool_choice not found in bound_llm.kwargs"
    ...
    assert "tools" in bound_llm_none.kwargs, "Tools not found in bound_llm_none.kwargs (tool_choice='none')"
    assert bound_llm_none.kwargs["tools"] == [SimpleToolSchema], "Tools incorrect in bound_llm_none.kwargs (tool_choice='none')"
    assert bound_llm_none.kwargs.get("tool_choice") == "none", "tool_choice was not 'none' in bound_llm_none.kwargs"
    ...
    configured_mock_llama_clients.async_client.chat.completions.create.assert_called_once()
    call_args_none = configured_mock_llama_clients.async_client.chat.completions.create.call_args
    # Assert that 'tool_choice' is NOT passed to the Llama API
...
@pytest.mark.asyncio
async def test_langgraph_serialization_compatibility(configured_mock_llama_clients: MockClients): # Updated fixture name
    ...
    mock_raw_tc = MagicMock(id="lg_tool_123", function=MagicMock(name="langgraph_tool", arguments=json.dumps({"lg_key": "lg_value"})))
    ...
    mock_structured_response.to_dict = MagicMock(return_value={"completion_message": mock_completion_msg.to_dict(), "x_request_id": "mock_req_langgraph"})
    ...
    configured_mock_llama_clients.async_client.chat.completions.create.side_effect = mock_create_for_langgraph_test
