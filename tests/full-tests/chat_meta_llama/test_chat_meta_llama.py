...
def chat_model(mock_llama_client: MagicMock) -> ChatMetaLlama:
    return ChatMetaLlama(client=mock_llama_client, model_name="test-llama-model")
...
class TestLcMessageToLlamaMessageParam:
    def test_human_message(self):
        ...
        expected_llama_message: MessageParam = {
            "role": "user",
            "content": "Hello there!",
        }
    ...
    def test_ai_message_none_content(self):
        ...
        lc_message = AIMessage(
            content="",
            tool_calls=[{"id": "t1", "name": "tool", "args": {"arg1": "val1"}}],
        )
    ...
    def test_ai_message_with_tool_calls(self):
        ...
        lc_message = AIMessage(
            content="I need to check the weather.", tool_calls=tool_calls
        )
    ...
    def test_tool_message_dict_content(self):
        ...
        lc_message = ToolMessage(
            content=json.dumps(dict_content), tool_call_id="call_456"
        )
...
class TestLcToolToLlamaToolParam:
    ...
    def test_tool_decorated_function_implicit_args_schema(self):
        ...
        assert (
            llama_tool_param["function"]["description"]
            == "Gets weather using a function."
        )
    ...
    def test_tool_missing_attributes_raises_error(self):
        ...
        with pytest.raises(
            ValueError,
            match="LangChain tool must have name, description, and args_schema",
        ):
...
class ChatMetaLlamaInitialization:
...
class ChatMetaLlamaWithTools:
    ...
    def test_model_with_tools(self, mock_llama_client):
        ...
        # Initialize model
        model = ChatMetaLlama(
            client=mock_llama_client, model_name="Llama-3.3-8B-Instruct"
        )
        ...
        assert any(
            "GetWeatherTool" in name or "get_weather_pydantic" in name
...
# Core method tests
class ChatMetaLlamaCoreMethods:
    ...
    def test_generate_with_tool_calls(self, chat_model, mock_llama_client):
        ...
        assert (
            result.generations[0].message.tool_calls[0]["args"]["location"]
            == "New York"
        )

    @pytest.mark.skip(
        reason="Streaming synchronous tests are difficult to mock reliably - covered by async tests instead"
    )
        ...
        with patch.object(chat_model, "_astream") as mock_astream:
            ...
            async def tool_call_generator():
                ...
                yield ChatGenerationChunk(
                    message=AIMessageChunk(content="Let me check the weather")
                )
...
class ChatMetaLlamaErrorHandling:
    def test_api_error_handling(self, chat_model, mock_llama_client):
        ...
        mock_llama_client.chat.completions.create.side_effect = APIError(
            message="API error", request=mock_request, body=mock_body
        )
    ...
    def test_invalid_tool_arguments(self, chat_model, mock_llama_client):
        ...
        if hasattr(result.generations[0].message, "invalid_tool_calls"):
            ...
            assert (
                "JSONDecodeError"
                in result.generations[0].message.invalid_tool_calls[0]["error"]
            )
...
class ChatMetaLlamaFeatures:
    ...
    def test_stop_reason_in_generation_info(self, chat_model, mock_llama_client):
        ...
        assert (
            result.generations[0].generation_info["finish_reason"] == "content_filter"
        )
    ...
    def test_empty_message_content(self, chat_model, mock_llama_client):
        ...
        mock_response = MagicMock()
        mock_response.completion_message.content = (
            "I didn't receive any input. How can I help you?"
        )
        mock_response.completion_message.tool_calls = []
        mock_response.completion_message.stop_reason = "stop"
        mock_response.metrics = []
    ...
    def test_ai_message_with_tool_calls_none_content(
        self, chat_model, mock_llama_client
    ):
...
class TestMultimodalHandling:
    ...
    def test_multimodal_content_preparation(self, mock_llama_client):
        ...
        mock_response.completion_message.content[
            0
        ].text = "This is a response about the image."
        mock_response.completion_message.tool_calls = []
        mock_response.completion_message.stop_reason = "stop"
        mock_response.metrics = []
        ...
        assert len(result.generations) == 1
        assert (
            "This is a response about the image."
            in result.generations[0].message.content
        )
...
def test_root_validator_with_env_vars():
    ...
    with patch.dict(
        os.environ,
        {"META_API_KEY": "env_key", "META_API_BASE_URL": "https://env.api"},
        clear=True,
    ):
        ...
        try:
            ...
            with patch(
                "integration.chat_meta_llama.LlamaAPIClient"
            ...
            pytest.fail(
                f"Failed to create ChatMetaLlama with environment variables: {e}"
            )
    ...
    mock_chunks = [
        MagicMock(
            ...
            tool_calls=[
                MagicMock(
                    id="call_1",
                    function=MagicMock(
                        name="get_weather", arguments=json.dumps({"location": "Lon"})
                    ),
                )
            ],
        ),
        MagicMock(
            ...
            tool_calls=[
                MagicMock(
                    id="call_1",
                    function=MagicMock(arguments=json.dumps({"location": "don"})),
                )
            ],
        ),
    ]
...
async def test_async_stream_termination(chat_model, mock_llama_client):
    ...
    mock_chunks = [
        MagicMock(type="completion_message_delta", delta=MagicMock(content="Final")),
        MagicMock(
    ...
    mock_llama_client.chat.completions.create = AsyncMock()
    mock_llama_client.chat.completions.create.return_value = AsyncMockIterator(
        mock_chunks
    )
...
def test_invalid_tool_response_handling(chat_model, mock_llama_client):
    ...
    tool_call.function = MagicMock(name="broken_tool", arguments="invalid{json")
    mock_response.completion_message = MagicMock(
        content=None, tool_calls=[tool_call], stop_reason="tool_calls"
    )
    mock_llama_client.chat.completions.create.return_value = mock_response
...
def test_complex_stream_handling(chat_model, mock_llama_client):
    ...
    mock_chunks = [
        MagicMock(
            ...
            delta=MagicMock(content="Let me", tool_calls=[]),
        ),
        MagicMock(
            type="tool_calls_generation_chunk",
            tool_calls=[
                MagicMock(
                    id="call_123",
                    function=MagicMock(
                        name="get_weather", arguments=json.dumps({"location": "London"})
                    ),
                )
            ],
        ),
...
def test_parameter_validation_edge_cases():
    ...
    with pytest.raises(ValueError, match="max_tokens must be at least 1"):
        ChatMetaLlama(
            client=MagicMock(), model_name="Llama-3.3-8B-Instruct", max_tokens=0
        )
