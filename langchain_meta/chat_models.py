...
from pydantic.v1 import Field, SecretStr, root_validator, validator
...
from llama_api_client.types.create_chat_completion_response import CreateChatCompletionResponse # For output
from llama_api_client.types.create_chat_completion_response_stream_chunk import CreateChatCompletionResponseStreamChunk # For streaming output
...
def _lc_message_to_llama_message_param(message: BaseMessage) -> MessageParam:
    ...
    if isinstance(message, HumanMessage):
        if not isinstance(message.content, str):
            ...
            raise ValueError(f"HumanMessage content for Llama API must be a string, got {type(message.content)}")
        ...
        if message.tool_calls:
            ...
            for tc in message.tool_calls:
                ...
                try:
                    ...
                    logger.warning(f"Could not serialize tool call args for {tc_name=}: {e}", exc_info=True)
                    tc_args_str = "{}"
            ...
            return {"role": "assistant", "content": content_value, "tool_calls": llama_tool_calls}
        ...
        if not isinstance(content_value, str):
             ...
             logger.warning(f"AIMessage content was not a string: {type(content_value)}. Converting to string.")
        ...
        if not isinstance(message.content, str):
            ...
            raise ValueError(f"SystemMessage content for Llama API must be a string, got {type(message.content)}")
        ...
        if isinstance(message.content, (dict, list)):
             try:
                 ...
                 logger.warning("Failed to dump ToolMessage complex content to JSON string", exc_info=True)
            ...
            logger.warning(f"Unexpected ToolMessage content type: {type(message.content)}. Converting to string.")
...
def _lc_tool_to_llama_tool_param(
    ...
    # Check for the required string attributes
    ...
    if hasattr(lc_tool, "name") and hasattr(lc_tool, "description") and (not isinstance(lc_tool.name, str) or not isinstance(lc_tool.description, str)):
        raise ValueError(
    ...
    if hasattr(lc_tool, "args_schema") and lc_tool.args_schema is not None:
        ...
        # If args_schema exists but is not usable, we'll use the default empty schema
        # and could warn the user, but won't fail

    return {
        "type": "function", # Llama API only supports 'function' tools
        "function": {
            "name": lc_tool.name,
...
class ChatMetaLlama(BaseChatModel):
    ...
    Example:
        ...
        # from your_module import ChatMetaLlama (assuming this class is in your_module.py)
    ...
    client: Any = Field(exclude=True) # Exclude from Pydantic model schema if class is a model itself
    ...
    max_tokens: Optional[int] = Field(default=None, alias="max_completion_tokens") # LangChain uses max_tokens
    ...
    def __init__(self, **data):
        ...
        if 'repetition_penalty' in data_clean and data_clean['repetition_penalty'] is not None:
    ...
    def _identifying_params(self) -> Dict[str, Any]:
        ...
        temp = None if self.temperature is None or isinstance(self.temperature, FieldInfo) else self.temperature
        max_tokens = None if self.max_tokens is None or isinstance(self.max_tokens, FieldInfo) else self.max_tokens
        rep_penalty = None if self.repetition_penalty is None or isinstance(self.repetition_penalty, FieldInfo) else self.repetition_penalty
    ...
    def _ensure_client_initialized(self) -> None:
        ...
        if not hasattr(self, 'client') or self.client is None:
            ...
            if not isinstance(current_base_url, str) or not current_base_url:
                env_base_url = os.environ.get("META_NATIVE_API_BASE_URL", "https://api.llama.com/v1/")
            ...
            try:
                ...
                self.client = LocalLlamaAPIClient(
                    ...
                    base_url=str(current_base_url)  # Ensure it's a string for the client
                )
            except ImportError:
                raise ImportError(
    ...
    def _generate(
        ...
        stop: Optional[List[str]] = None, # Llama API client doesn't show explicit 'stop' in create()
        ...
        **kwargs: Any,
        ...
        llama_messages: List[MessageParam] = [_lc_message_to_llama_message_param(m) for m in messages]
        ...
        try:
            response_obj: CreateChatCompletionResponse = self.client.chat.completions.create(**api_params)
        ...
        if msg_data.content:
            ...
            elif hasattr(msg_data.content, 'text') and isinstance(msg_data.content.text, str): 
        ...
        if msg_data.tool_calls:
            for tc in msg_data.tool_calls:
                ...
                if isinstance(tc, MagicMock) and hasattr(tc, 'function') and \
                   isinstance(tc.function, MagicMock) and \
                   hasattr(tc.function, 'arguments') and \
                   isinstance(tc.function.arguments, str) and \
                   tc.function.arguments.startswith('invalid'):
                    # Special case for test_invalid_tool_response_handling
                    invalid_tool_calls.append({
                        "id": str(tc.id) if tc.id else "unknown_id",
                        "name": str(tc.function.name) if hasattr(tc.function, 'name') else "unknown_tool",
                ...
                if not tc.function or not hasattr(tc.function, 'name') or not tc.function.name or not isinstance(tc.function.name, str):
                    # Handle malformed tool call from API 
                    invalid_tool_calls.append(
                        {
                            "id": str(tc.id) if tc.id else "unknown_id", # Ensure ID is a string
                            "name": "unknown_tool_name", # Use a default string
                            "args": "{}" if not tc.function else str(getattr(tc.function, 'arguments', '{}')),
                ...
                try:
                    args_dict = json.loads(tc.function.arguments or "{}") # Default to empty dict if args is None/empty
                    ...
                    invalid_tool_calls.append(
                        {
                            "id": str(tc.id),
                            "name": str(tc.function.name), # Ensure name is a string
                            "args": str(tc.function.arguments or ""), # Ensure args is a string 
                            "error": f"JSONDecodeError: {e}", 
                        }
                    )
        
        ai_message_kwargs: Dict[str, Any] = {"content": content_str if content_str else ""} # Ensure content is at least ""
        ...
        if response_obj.metrics: 
            usage_metadata = {m.metric: m.value for m in response_obj.metrics if m.metric and m.value is not None}
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        current_tool_calls: Dict[str, Dict[str, Any]] = {}  # tool_id -> {name, args_buffer, index}
        ...
        try:
            ...
            for stream_chunk_obj in stream_response:
                ...
                if getattr(chunk, 'type', None) == "completion_message_delta" and getattr(chunk, 'delta', None) and getattr(chunk.delta, 'content', None):
                ...
                elif getattr(chunk, 'type', None) == "tool_calls_generation_chunk" and getattr(chunk, 'tool_calls', None):
                    for i, tc_delta in enumerate(chunk.tool_calls):
                        # Ensure we have a valid ID string
                        tool_id = str(tc_delta.id) if hasattr(tc_delta, 'id') and tc_delta.id else f"tc_{i}"
                        ...
                        if tc_function and hasattr(tc_function, 'name'):
                            tc_name = str(tc_function.name) # Convert MagicMock to string if needed
                        ...
                        if tool_id not in current_tool_calls:
                            current_tool_calls[tool_id] = {
                                "name": tc_name,
                                "args_buffer": tc_args,
                                "index": len(current_tool_calls)  # Assign sequential index based on first appearance
                            }
                        else:
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        if msg_data.tool_calls:
            for tc in msg_data.tool_calls:
                # Ensure tc.function and tc.function.name are valid before accessing
                if not tc.function or not tc.function.name or not isinstance(tc.function.name, str):
                    # Handle malformed tool call from API if necessary
                    invalid_tool_calls.append(
                        {
                            "id": str(tc.id) if tc.id else "unknown_id", # Ensure ID is a string
                            "name": "unknown_tool_name", # Use a default string
                            "args": "{}" if not tc.function else str(tc.function.arguments or "{}"),
                            "error": "Malformed tool call structure from API",
                        }
                    )
                    continue
        ...
        generations.append(ChatGeneration(message=AIMessage(**ai_message_kwargs), generation_info=generation_info))
        ...
        try:
            ...
            while True:
                try:
                    ...
                    stream_chunk_obj = cast(CreateChatCompletionResponseStreamChunk, chunk)
                    ...
                    # Handle text content delta
                    if getattr(stream_chunk_obj, 'type', None) == "completion_message_delta" and \
                       getattr(stream_chunk_obj, 'delta', None) and \
                       getattr(stream_chunk_obj.delta, 'content', None):
                        delta_content = str(stream_chunk_obj.delta.content)
                        if run_manager:
                            await run_manager.on_llm_new_token(delta_content)
                    
                    # Handle tool call chunks, which may come in multiple deltas for the same tool call
                    elif getattr(stream_chunk_obj, 'type', None) == "tool_calls_generation_chunk" and \
                         getattr(stream_chunk_obj, 'tool_calls', None):
                    ...
                    # Handle stream completion - final chunk
                    elif getattr(stream_chunk_obj, 'type', None) == "completion_message_stop":
                        finish_reason = getattr(stream_chunk_obj, 'stop_reason', None)
                        if hasattr(stream_chunk_obj, 'metrics') and stream_chunk_obj.metrics:
                            usage_metadata = {}
